...
In this chapter you will open a terminal, see which shells it can run, read a prompt, and learn to use the window around the command line.
...
- the **terminal**, the **shell**, and the **operating system**, and which job belongs to which;
- the **prompt** and the **cursor**;
- the shells this book uses: Bash, zsh, PowerShell, and Command Prompt;
- **tabs**, **panes**, and **focus**;
- copy, paste, **scrollback**, and terminal search;
- **command history** as a shell feature;
- **terminal profiles** and ending a session.
...
|---|---|---|
| Terminal | Display output, accept keyboard input, copy text | Windows Terminal, macOS Terminal, GNOME Terminal, xterm |
...
Its name might be Terminal, GNOME Terminal, Console, Konsole, or xterm.
Linux desktops do not all ship the same application or assign the same shortcuts.
The figure above uses **xterm**; the Linux shortcut examples in this chapter are specifically for **GNOME Terminal**.
...
Either one lets you follow the UNIX track on Windows.
...
We will explore directories properly in Chapter 3.
...
In the console boxes in this book, **bold lines** are commands you type, and ordinary lines are output.
The `$ ` at the start of a command is our prompt marker, even for Windows shells; do not type it.

Click inside the terminal window so it has focus, type the following line, and press Enter (Return on a Mac):
...
The line `console-ready` underneath the command is its output.
...
In the HTML edition, **Copy commands** copies only the commands from a console box, without the `$ ` markers or output.
Read each command before pasting it into your shell.
...
Run `echo another-line` once.
At the next prompt, press **Up Arrow** to recall that command, then press **Enter** to run it again.
Repeat this a few times without retyping the command.
...
## Recall and Edit a Command

The Up Arrow you used above relies on **command history**: the commands the shell remembers so you can recall and edit them.
History belongs to the shell, not the terminal.
Scrollback can include commands, output, and error messages.
History generally contains commands, not their output.

You can also edit a recalled command before running it.
With the usual shell key bindings, try this at an empty prompt:

1. Press **Up Arrow** to bring back `echo another-line`. The cursor starts at the end.
2. Press **Left Arrow** four times to move just before `line`.
3. Type `new-`. The command now reads `echo another-new-line`.
4. Press **Enter**. The shell runs the whole command, even with the cursor in the middle of the line.

The edited command and its output are:
...
Left and Right Arrow move the cursor without changing the text.
Typing inserts text at the cursor; **Backspace** removes the character just before it.
You can press Up Arrow repeatedly to reach older commands and **Down Arrow** to move toward newer ones.
Recalling a command puts it on the line for editing; it only runs when you press Enter.
Chapter 4 covers these editing keys, their alternatives, and how to search history.
...
You can also **pimp your shell prompt**.
Explore your shell's prompt customization: colors, symbols, and the information it displays.
Can you make your current directory easier to spot, or make different shell sessions easier to tell apart?
Aim for something that looks good and helps you work without crowding out the command you are typing.
We will dig into prompt configuration in Chapter 10.
...
1. **What does this do?** In a fresh tab, enter `echo first-tab`, open a second tab, and press Up Arrow there. Predict what appears before you press the key.
2. **What is the problem?** A classmate types `$ echo hello` exactly as printed in a tutorial and the shell reports an error about `$`. Explain what went wrong and what they should have typed.
3. **Why is it like this?** Scrolling back through output does not let you rerun the command you can see. Explain which program owns the scrollback and which owns the command history, and why scrolling changes only one of them.
4. **Why is it like this?** Changing the font in your terminal's settings changes every shell you open in it, but the shells still speak different languages. Explain which piece owns the font and which owns the language.
5. **How do you** find out which shell a terminal profile starts before you type anything? Describe where to look in your terminal's settings, then open that profile and confirm your answer from the prompt.
6. **How do you** copy an error message out of the terminal without accidentally sending Ctrl+C to the running program? Describe the steps for your terminal.
