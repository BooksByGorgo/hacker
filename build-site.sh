#!/bin/bash
# Build the GitHub Pages site under docs/ from the book in n00b2punk/.
#
# For the book it: runs the Makefile to build the PDFs, converts every
# chapter and appendix to a Jekyll page under docs/n00b2punk/, copies the
# PDFs and images next to them, builds one self-contained single-page HTML
# of the whole book, and writes the chapter list include that
# docs/n00b2punk.md pulls in.
set -euo pipefail
shopt -s nullglob
...
build_book() {
    ...
    # Chapter pages reference images/ relative to themselves (the same
    # relative path the standalone HTML and the PDF use), so copy the
    # book's images next to the generated pages.
    mkdir -p "$DOCS/$dest_subdir/images"
    cp "$src_dir"/images/*.png "$DOCS/$dest_subdir/images/"
...
build_book n00b2punk  n00b2punk  "n00b to console punk"  "n00b2punk"
